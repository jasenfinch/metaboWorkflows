...
For instance, the `FIE-HRMS_fingerprinting` workflow can be retrieved using the following. 
The addition of the vector containing the file paths allows for automatic parameter detection.

```{r parameters}
parameters <- workflowParameters('FIE-HRMS_fingerprinting',files = files)
parameters
```
...
```{r, showFlags}
wp <- workflowParameters('FIE-HRMS_fingerprinting')
flags(wp)
```
