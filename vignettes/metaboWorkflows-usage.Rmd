...
parameters <- workflowParameters('FIE-HRMS fingerprinting',files = files,nCores = 2)
```

```{r parameters,eval=FALSE}
parameters <- workflowParameters('FIE-HRMS fingerprinting',files = files)
parameters
```
...
```{r, showFlags}
wp <- workflowParameters('FIE-HRMS fingerprinting')
flags(wp)
```
...
A number of plotting methods are available for processed and pre-treated data that are extentions of the functions found within the `binneR`, `profilePro` and `metabolyseR` packages.
